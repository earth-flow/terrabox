...
from ..data import list_toolkits, list_tools, get_tool, get_handler
from ..db.session import get_db
from ..db import models as m
from .deps import current_user_from_api_key
...
def get_tools(toolkit: str | None = None, connected_account_id: str | None = None, current_user: m.User = Depends(current_user_from_api_key), db: Session = Depends(get_db)):
    ...
    if connected_account_id:
        ...
        if not acc or acc.user_id_fk != current_user.id:
...
def get_tool_detail(slug: str, current_user: m.User = Depends(current_user_from_api_key), db: Session = Depends(get_db)):
...
def get_toolkits(current_user: m.User = Depends(current_user_from_api_key), db: Session = Depends(get_db)):
...
def execute_tool(slug: str, payload: ExecuteRequest, current_user: m.User = Depends(current_user_from_api_key), db: Session = Depends(get_db)):
    ...
    if spec.requires_connection:
        ...
        if connected_account_id:
            account = db.query(m.ConnectedAccount).filter_by(id=int(connected_account_id)).first()
            if account is None or account.user_id_fk != current_user.id or account.toolkit != spec.toolkit:
            ...
            accounts = db.query(m.ConnectedAccount).filter_by(user_id_fk=current_user.id, toolkit=spec.toolkit).all()
            if len(accounts) == 1:
                account = accounts[0]
            else:
