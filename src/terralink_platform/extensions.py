...
from .data import register_tool as _register_tool, register_toolkit as _register_toolkit
from .models import ToolSpec, Toolkit


# Global registry to store tool handlers
...
class Registrar:
    ...
    def toolkit(self, name: str, description: str, version: str = "1.0"):
        ...
        """
        toolkit_meta = Toolkit(name=name, description=description, version=version)
        _register_toolkit(toolkit_meta)
    ...
    def tool(self, spec: ToolSpec, handler: Callable):
        ...
        """
        # Register the tool spec in the data layer
        _register_tool(spec)
        # Store the handler in our global registry
        _TOOL_HANDLERS[spec.slug] = handler
...
def get_handler(slug: str) -> Callable | None:
    ...
    """
    return _TOOL_HANDLERS.get(slug)
...
def load_builtin_toolkits():
    ...
    for py in base.glob("*.py"):
        ...
        try:
            ...
            print(f"Warning: Failed to load builtin toolkit {py.stem}: {e}")
