...
def bash_execute_handler(arguments: dict, context: dict, account=None) -> Dict[str, Any]:
    ...
    Returns:
        Dict containing command execution results or error information
    """
    try:
        # Extract commands from arguments
        commands = arguments.get("commands", "")
        timeout = int(arguments.get("timeout", TIMEOUT))
        ...
        try:
            # Create bash session
            session = BashSession(temp_dir, use_firejail=False)  # Use safer option
            ...
            all_commands = "\n".join(commands)
            forbidden_detected = check_forbidden_commands(all_commands)
            if forbidden_detected:
                return {
                    "success": False,
                    "error": f"Forbidden commands detected: {', '.join(forbidden_detected)}",
                    "commands": commands
                }
            
            # Execute commands
            result = session.execute_command_like_shell(commands, timeout)
            ...
            return {
                ...
                "commands": commands,
                "output": result.strip(),
                "history": history[-10:] if len(history) > 10 else history,  # Last 10 commands
        ...
        return {
            "success": False,
            "error": f"Bash execution failed with error: {str(e)}",
            "commands": arguments.get("commands", [])
        }
...
def setup(registrar):
    ...
    """
    # Register the bash toolkit
    registrar.toolkit(
        name="bash",
        description="Execute bash commands in secure, persistent shell sessions with file operations restricted to temporary directories",
        version="1.0.0"
    )
    
    # Register the bash execute tool
    bash_execute_spec = ToolSpec(
        ...
        parameters={
            ...
            "properties": {
                "commands": {
                    "type": "string",
                    "description": "Bash commands to execute (can be a single command or multiple commands separated by newlines)"
                },
                "timeout": {
                    "type": "integer", 
    ...
    registrar.tool(bash_execute_spec, bash_execute_handler)
